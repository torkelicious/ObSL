...
static Value deserialize_value(ASTDeserializer &deser) {
  ...
  default:
    throw std::runtime_error(
        "Malformed binary AST: Unknown value discriminant");
  }
}

static Param deserialize_param(ASTDeserializer &deser) {
  Param param;
  param.name = std::string(deser.read_string_view());
...
static StructField deserialize_struct_field(ASTDeserializer &deser) {
  StructField field;
  field.name = std::string(deser.read_string_view());
...
std::unique_ptr<Expr> ASTDeserializer::deserialize_expr() {
  ...
  case ExprType::Binary: {
    ...
    return std::make_unique<BinaryExpr>(std::move(left), dummy_oprt,
                                        std::move(right));
  }
  case ExprType::Logical: {
    ...
    Token dummy_oprt{};
    dummy_oprt.type = oprt;
    return std::make_unique<LogicalExpr>(std::move(left), dummy_oprt,
                                         std::move(right));
  }
  ...
  case ExprType::Index: {
    ...
    return std::make_unique<IndexExpr>(std::move(callee), bracket,
                                       std::move(index));
  }
  ...
  case ExprType::TypeCheck: {
    ...
    return std::make_unique<TypeCheckExpr>(std::move(left),
                                           std::move(type_name));
  }
  }
...
std::unique_ptr<Stmt> ASTDeserializer::deserialize_stmt() {
  ...
  case StmtType::Function: {
    ...
    // FunctionStmt expects the body as BlockStmt so downcast after
    // deserialisation.
    ...
    Token dummy_name{};
    dummy_name.lexeme = name;
    return std::make_unique<FunctionStmt>(dummy_name, std::move(params),
                                          std::move(block_body));
  }
  case StmtType::If: {
  ...
  case StmtType::Foreach: {
    ...
    return std::make_unique<ForeachStmt>(dummy_var, std::move(iterable),
                                         std::move(body));
  }
  ...
  case StmtType::TryCatch: {
    ...
    return std::make_unique<TryCatchStmt>(std::move(try_block), dummy_exc,
                                          std::move(catch_block));
  }
  case StmtType::Struct: {
    const std::string name = std::string(read_string_view());
