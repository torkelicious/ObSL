...
namespace ObSL {
    ...
    std::vector<Token> Lexer::tokenize() {
        ...
        tokens.push_back(Token{TokenType::EOF_, "", static_cast<uint16_t>(line), static_cast<uint16_t>(column),
    ...
    Token Lexer::read_string() {
        ...
        advance();
        return Token{TokenType::STRING, std::string_view(&source[start_pos], current - start_pos),
    ...
    Token Lexer::read_operator_or_symbol() {
        ...
        switch (advance()) {
            ...
            case '<':
                ...
                if (peek() == '<') {
                    advance();
                    return Token{TokenType::LESS_LESS,
                                 "<<",
                                 static_cast<uint16_t>(line),
                                 static_cast<uint16_t>(start_col),
                                 static_cast<uint32_t>(start_pos),
