...
public:
  void register_modules(Interpreter &interpreter) override {
    ...
    interpreter.define_native(
        "to_fixed", [](const double val, const double decimals) -> std::string {
      ...
      try {
        ...
        if (idx != str.length()) {
          throw std::runtime_error(
              "Conversion Error: String contains non-numeric characters.");
        }
        return val;
      } catch (const std::invalid_argument &) {
        throw std::runtime_error("Conversion Error: Cannot convert string '" +
                                 str + "' to a number.");
...
public:
  void register_modules(Interpreter &interpreter) override {
    interpreter.define_native("sqrt", [](const double x) -> double {
      if (x < 0.0) {
        throw std::runtime_error(
            "Math Error: Cannot calculate square root of a negative number.");
      }
      return std::sqrt(x);
    });
    ...
    // base^exponent
    interpreter.define_native(
        "pow", [](const double base, const double exponent) -> double {
    ...
    // min/max
    interpreter.define_native("min",
                              [](const double a, const double b) -> double {
                                return std::min(a, b);
                              });
    interpreter.define_native("max",
                              [](const double a, const double b) -> double {
                                return std::max(a, b);
                              });
    ...
    interpreter.define_native(
        "clamp",
        [](const double value, const double min, const double max) -> double {
          if (min > max)
            throw std::runtime_error(
                "Math Error: clamp() min cannot be greater than max.");
          return std::clamp(value, min, max);
        });

    // rounding utilities
    interpreter.define_native(
        "floor", [](const double x) -> double { return std::floor(x); });
    interpreter.define_native(
        "ceil", [](const double x) -> double { return std::ceil(x); });
    interpreter.define_native(
        "round", [](const double x) -> double { return std::round(x); });
    interpreter.define_native(
        "abs", [](const double x) -> double { return std::abs(x); });

    // trigonometry (radians)
    interpreter.define_native(
        "sin", [](const double x) -> double { return std::sin(x); });
    interpreter.define_native(
        "cos", [](const double x) -> double { return std::cos(x); });
    interpreter.define_native(
        "tan", [](const double x) -> double { return std::tan(x); });
    interpreter.define_native("atan2",
                              [](const double y, const double x) -> double {
                                return std::atan2(y, x);
                              });

    // get pi
    interpreter.define_native("pi",
                              []() -> double { return std::numbers::pi; });

    // degree/rad conversions
    interpreter.define_native("rad", [](const double deg) -> double {
      return deg * (std::numbers::pi / 180.0);
    });
    interpreter.define_native("deg", [](const double rad) -> double {
    ...
    interpreter.define_native(
        "lerp",
        [](const double start, const double end, const double t) -> double {
...
public:
  void register_modules(Interpreter &interpreter) override {
    ...
    interpreter.define_native(
        "starts_with",
        [](const std::string &str, const std::string &prefix) -> bool {
    ...
    interpreter.define_native(
        ...
        [](const std::string &str, const double start,
          ...
          if (std::isnan(start) || std::isnan(length) || str.empty())
            return "";
          const double safe_start =
              std::clamp(start, 0.0, static_cast<double>(str.length()));
          const auto s = static_cast<size_t>(safe_start);
          const double safe_len =
              std::clamp(length, 0.0, static_cast<double>(str.length() - s));
    ...
    interpreter.define_native("trim", [](std::string str) -> std::string {
      const auto start = std::ranges::find_if_not(
          str, [](const unsigned char ch) { return std::isspace(ch); });
...
public:
  void register_modules(Interpreter &interpreter) override {
    ...
    interpreter.define_native("exit", [](const double status_code) -> double {
      throw std::runtime_error(
          std::format("Script exited with code: {}", status_code));
      return 0.0;
    });
    ...
    interpreter.define_native(
        "get_env", [](const std::string &var_name) -> std::string {
    ...
    interpreter.define_native("file_exists",
    ...
    interpreter.define_native(
        "get_file_ext", [](const std::string &path) -> std::string {
    ...
    interpreter.define_native(
        "read_file", [](const std::string &path) -> std::string {
          ...
          if (!file.is_open()) {
            throw std::runtime_error(
                "File Error: Could not open file for reading: " + path);
          }
    ...
    interpreter.define_native(
        ...
        [](const std::string &path, const std::string &content) -> bool {
          ...
          if (!file.is_open()) {
            throw std::runtime_error(
                "File Error: Could not open file for writing: " + path);
          }
          file << content;
          return true;
        });
    ...
    interpreter.define_native(
        "sleep_thread", [](const double seconds) -> double {
          if (seconds > 0.0) {
...
public:
  void register_modules(Interpreter &interpreter) override {
    ...
    interpreter.define_native(
        "regex_match",
        [](const std::string &text, const std::string &pattern) -> bool {
          try {
...
public:
  void register_modules(Interpreter &interpreter) override {
    // string type name of any given script value ("null", "number", "string",
    // etc.)
    ...
    interpreter.define_native(
        "has_field",
        [](ObSLObject *obj, const std::string &field_name) -> bool {
          if (!obj)
            return false;
    ...
    interpreter.define_native(
        "get_fields", [&interpreter](ObSLObject *obj) -> ObSLArray * {
          const auto arr = interpreter.gc.allocate<ObSLArray>();
          if (obj) {
    ...
    // ReSharper disable once CppParameterMayBeConstPtrOrRef
    interpreter.define_native("get_arity",
                              [](ObSLCallable *callable) -> double {
                                if (!callable)
                                  return -1.0;
