...
void ScriptWorker::execute(const std::vector<std::unique_ptr<Stmt>> &ast,
...
Value ScriptWorker::GetVal(const std::string &name,
                           const std::shared_ptr<Environment> &env) {
  return env->get(name);
}

void ScriptWorker::SetVal(const std::string &name, const Value &val,
                          const std::shared_ptr<Environment> &env) {
  env->define(name, val);
}
