...
namespace ObSL {
    ScriptWorker::ScriptWorker(const std::string &scriptroot)
        : m_Interpreter(scriptroot), m_Globals(m_Interpreter.get_global_environment()) {
    ...
    std::shared_ptr<Environment> ScriptWorker::copy_globals() { return std::make_shared<Environment>(m_Globals); }
} // namespace ObSL
