...
using Value = std::variant<std::monostate, bool, double, std::string,
                           ObSLCallable *, ObSLArray *, ObSLObject *>;

void mark_value(const Value &val);

struct ObSLObject : GCObject {
...
struct BinaryExpr : Expr {
  ...
  BinaryExpr(std::unique_ptr<Expr> left, const Token &oprt,
             std::unique_ptr<Expr> right)
  ...
  [[nodiscard]] std::string to_string() const override {
    return std::format("({} {} {})", static_cast<int>(oprt_type),
...
struct LogicalExpr : Expr {
  ...
  LogicalExpr(std::unique_ptr<Expr> left, const Token &oprt,
              std::unique_ptr<Expr> right)
      : left(std::move(left)), oprt_type(oprt.type), right(std::move(right)) {}
...
struct VariableExpr : Expr {
  std::string name;
  Token token; // position of the identifier, for error reporting

  explicit VariableExpr(const Token &name)
      : name(std::string(name.lexeme)), token(name) {}

  explicit VariableExpr(const std::string_view name)
      : name(std::string(name)),
        token(Token{TokenType::IDENTIFIER, this->name, 0, 0, 0, 0}) {}
...
struct AssignmentExpr : Expr {
  std::string name;
  Token token; // position of the assigned identifier, for error reporting
  std::unique_ptr<Expr> value;

  AssignmentExpr(const Token &name, std::unique_ptr<Expr> value)
      : name(std::string(name.lexeme)), token(name), value(std::move(value)) {}
...
struct UpdateExpr : Expr {
  ...
  UpdateExpr(const std::string_view name, const Token &oprt, const bool prefix)
      : name(std::string(name)),
        token(Token{TokenType::IDENTIFIER, this->name, 0, 0, 0, 0}),
        oprt_type(oprt.type), is_prefix(prefix) {}

  UpdateExpr(const Token &name, const Token &oprt, const bool prefix)
      : name(std::string(name.lexeme)), token(name), oprt_type(oprt.type),
        is_prefix(prefix) {}
  ...
  [[nodiscard]] std::string to_string() const override {
    ...
    if (is_prefix)
      return std::format("({}{})", oprt_str, name);
...
struct IndexExpr : Expr {
  ...
  IndexExpr(std::unique_ptr<Expr> callee, const Token &bracket,
            std::unique_ptr<Expr> index)
...
struct IndexAssignmentExpr : Expr {
  std::unique_ptr<Expr> callee;
  Token bracket;
  std::unique_ptr<Expr> index;
  std::unique_ptr<Expr> value;

  IndexAssignmentExpr(std::unique_ptr<Expr> callee, const Token &bracket,
                      std::unique_ptr<Expr> index, std::unique_ptr<Expr> value)
      : callee(std::move(callee)), bracket(bracket), index(std::move(index)),
        value(std::move(value)) {}

  [[nodiscard]] ExprType type() const noexcept override {
    return ExprType::IndexAssignment;
  }
...
struct GetExpr : Expr {
  ...
  GetExpr(std::unique_ptr<Expr> obj, const Token &name)
      : obj(std::move(obj)), name(std::string(name.lexeme)) {}
...
struct SetExpr : Expr {
  ...
  SetExpr(std::unique_ptr<Expr> obj, const std::string_view name,
          std::unique_ptr<Expr> value)
      : obj(std::move(obj)), name(std::string(name)), value(std::move(value)) {}

  SetExpr(std::unique_ptr<Expr> obj, const Token &name,
          std::unique_ptr<Expr> value)
      : obj(std::move(obj)), name(std::string(name.lexeme)),
        value(std::move(value)) {}
...
struct UsingStmt : Stmt {
  ...
  [[nodiscard]] StmtType type() const noexcept override {
    return StmtType::Using;
  }
...
struct FunctionStmt : Stmt {
  ...
  FunctionStmt(const Token &name, std::vector<Param> params,
               std::unique_ptr<BlockStmt> body)
      : name(std::string(name.lexeme)), params(std::move(params)),
        body(std::move(body)) {}
...
struct CaseBranch {
  ...
  CaseBranch(std::unique_ptr<Expr> match_value,
             std::vector<std::unique_ptr<Stmt>> statements)
...
struct ForeachStmt : Stmt {
  ...
  ForeachStmt(const Token &loop_var, std::unique_ptr<Expr> iterable,
              std::unique_ptr<Stmt> body)
      : loop_var(std::string(loop_var.lexeme)), iterable(std::move(iterable)),
        body(std::move(body)) {}
...
struct VarStmt : Stmt {
  ...
  VarStmt(const Token &name, std::unique_ptr<Expr> initializer)
      : name(std::string(name.lexeme)), initializer(std::move(initializer)) {}
...
struct TryCatchStmt : Stmt {
  ...
  TryCatchStmt(std::unique_ptr<BlockStmt> try_body, const Token &exception_var,
      ...
      : try_body(std::move(try_body)),
        exception_var(std::string(exception_var.lexeme)),
        catch_body(std::move(catch_body)) {}

  [[nodiscard]] StmtType type() const noexcept override {
    return StmtType::TryCatch;
  }

  [[nodiscard]] std::string to_string() const override {
    return std::format("[TryCatchStmt: try {} catch({}) {}]\n",
                       try_body->to_string(), exception_var,
...
struct StructStmt : Stmt {
  ...
  StructStmt(const Token &name, std::vector<StructField> fields)
      : name(std::string(name.lexeme)), fields(std::move(fields)) {}

  StructStmt(std::string name, std::vector<StructField> fields)
  ...
  [[nodiscard]] std::string to_string() const override {
    return std::format("[StructStmt: {}]\n", name);
  }
};

struct ObSLStruct : ObSLCallable {
  const StructStmt *declaration;

  explicit ObSLStruct(const StructStmt *declaration)
  ...
  [[nodiscard]] int min_arity() const override { return 0; }

  Value call(Interpreter *interpreter, const std::vector<Value> &arguments,
             const Token &call_token) override;

  [[nodiscard]] std::string to_string() const override {
    return std::format("<struct {}>", declaration->name);
  }
};
} // namespace ObSL
