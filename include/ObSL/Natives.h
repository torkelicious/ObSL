...
public:
  ...
  Value call(Interpreter *interpreter, const std::vector<Value> &arguments,
    ...
    if (arguments.size() != m_arity) {
      throw RuntimeError(
          call_token,
          std::format("Native function expects {} arguments but got {}.",
    ...
    try {
      ...
      throw RuntimeError(call_token, std::format("Native function '{}': {}",
                                                 m_name, e.what()));
    }
  }
  ...
  if constexpr (std::is_same_v<DecayedTarget, Value>) {
    ...
    if (!std::holds_alternative<DecayedTarget>(arg)) {
      throw NativeTypeError(
          std::format("Argument {}: expected type '{}', got '{}'.", index,
...
static Value call_native_helper(const F &body, const std::vector<Value> &args,
  ...
  if constexpr (std::is_void_v<typename Traits::return_type>) {
    body(unpack.template operator()<std::tuple_element_t<Is, ArgsTuple>>(
        args[Is])...);
    return std::monostate{};
  } else {
    return body(unpack.template operator()<std::tuple_element_t<Is, ArgsTuple>>(
        args[Is])...);
  }
}
} // namespace ObSL
