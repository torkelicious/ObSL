    ...
    public:
        ...
        Value call(Interpreter *interpreter, const std::vector<Value> &arguments, const Token &call_token) override {
            if (arguments.size() != m_arity) {
                throw RuntimeError(call_token, std::format("Native function expects {} arguments but got {}.", m_arity,
                                                           arguments.size()));
            }
            try {
        ...
        [[nodiscard]] std::string to_string() const override { return std::format("<native fn {}>", m_name); }
