...
using ModuleLoader =
    std::function<std::optional<ModuleResult>(const std::string &)>;
...
public:
  ...
  explicit Interpreter(const std::string &scriptrootpath =
  ...
  void register_environment(const std::shared_ptr<Environment> &env) {
    ...
    if (++m_env_insert_count % prune_interval == 0) {
      std::erase_if(all_environments, [](const std::weak_ptr<Environment> &wp) {
        return wp.expired();
      });
    }
  ...
  acquire_environment(std::shared_ptr<Environment> enclosing);
  ...
  void set_module_loader(ModuleLoader loader) {
    m_module_loader = std::move(loader);
  }
...
private:
  ...
  // before other members that might use them during construction (e.g.,
  // StdLib).
  ...
  static void check_number_operands(TokenType oprt, const Value &lhs,
                                    const Value &rhs);
...
public:
  ObSLFunction(const FunctionStmt *declaration,
               std::shared_ptr<Environment> closure)
...
void Interpreter::define_native(std::string name, F &&body) {
  ...
  if constexpr (std::is_pointer_v<DecayedF> &&
                std::is_function_v<std::remove_pointer_t<DecayedF>>) {
    using Traits = native_fn_traits<DecayedF>;
    auto wrapped = [body = std::forward<F>(body)](
                       Interpreter *, const std::vector<Value> &args) -> Value {
      return call_native_helper<DecayedF, Traits>(
          body, args, std::make_index_sequence<Traits::arity>{});
    };
    globals->define(name, gc.allocate<NativeFunction>(
                              Traits::arity, std::move(wrapped), name));
  } else {
