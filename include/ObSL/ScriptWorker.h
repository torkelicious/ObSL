...
public:
  ...
  // The AST is borrowed, not owned
  // it must outlive this call and remain alive for the lifetime of the
  // interpreter
  void execute(const std::vector<std::unique_ptr<Stmt>> &ast,
               std::shared_ptr<Environment> env);

  Value GetVal(const std::string &name,
               const std::shared_ptr<Environment> &env);

  void SetVal(const std::string &name, const Value &val,
              const std::shared_ptr<Environment> &env);
